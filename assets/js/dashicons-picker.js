...
( function ( $ ) {
	...
	$.fn.dashiconsPicker = function () {
		...
		var icons = [
			...
			'fullscreen-alt',
			'fullscreen-exit-alt', */
            'share', 'share-alt', 'share-alt2', 'rss', 'email', 'email-alt', 'email-alt2', 'networking', 'amazon', 'facebook', 'facebook-alt', 'google', 'instagram', 'linkedin', 'pinterest', 'podio', 'reddit', 'spotify', 'twitch', 'twitter', 'twitter-alt', 'whatsapp', 'xing', 'youtube'
		];
		...
		return this.each( function () {
			...
			function createPopup( button ) {
				...
				$( 'a', list ).on( 'click', function ( e ) {
                    ...
                    let thumbnail_image = document.createElement('span')
                   
             
                    thumbnail_image.className = 'p-0 m-0 flex items-center justify-center text-right'
                    thumbnail_image.classList.add(`dashicons`)
                    thumbnail_image.classList.add(`dashicons-${title}`)
                

                    if(document.querySelector('#social_icon_preview a span')){
                        document.querySelector('#social_icon_preview a span').className = thumbnail_image.className
                    }else if(document.querySelector('#social_icon_preview a img')){
                       
                        thumbnail_image.style.fontSize = window.getComputedStyle(document.querySelector('#social_icon_preview a img')).width
                    ...
                    document.querySelector('#socialNetworkIcon').value = `dashicons-${title}`
                    document.querySelector('#socialNetworkIcon').dataset.type = 'icon'
